...
PYTHON_PACKAGE 			:= confluentcloud_iam_serviceaccount
endif

ifndef RES_NAME
RES_NAME				:= "ConfluentCloud::IAM::ServiceAccount"
endif
...
zip:			package
				...
				cp confluentcloud-iam-serviceaccount.json schema.json
				zip -r9 $(PYTHON_PACKAGE).zip ResourceProvider.zip src inputs .rpdk-config schema.json confluentcloud-iam-serviceaccount.json
